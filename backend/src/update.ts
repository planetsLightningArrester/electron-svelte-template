...
export async function getUpdates(): Promise<Update | undefined> {
  ...
  const latestReleaseUrl = new URL('https://github.com/planetsLightningArrester/electron-svelte-template/releases/latest')
  try {
    ...
    if (match === null || match.length < 1 || match[1] === '') throw new Error(`Unexpected URL without version number: ${res.url}`)
      ...
      if (
        ...
        switch (process.platform) {
          case 'darwin':
            fileName = `Electron.Svelte.template-darwin-x64-${latestVersion}.zip`
            break
          case 'win32':
            fileName = `Electron.Svelte.template-win32-x64-${latestVersion}.zip`
            break
          case 'linux':
            fileName = `Electron.Svelte.template-linux-x64-${latestVersion}.zip`
            break
          default:
...
export async function installUpdate({ version, name, url }: Update): Promise<void> {
  ...
  const compressedFullPath = path.join(tempDir, `electron_svelte_template-${version}.zip`)
  ...
  switch (process.platform) {
    case 'darwin':
      // INFO: MacOS path is `Electron Svelte template.app/Contents/MacOS/electron_svelte_template`
